# -*- encoding: utf-8 -*-
"""Tests for agent collaboration protocols (WorkflowOrchestrator, etc.).

Run with: pytest lib/crewai/tests/content/agents/test_protocols.py -v
"""
...
from crewai.content.agents.protocols.handoff import (
    AgentHandoff,
    HandoffResult,
    HandoffStatus,
    QualityGateError,
)
from crewai.content.agents.protocols.context import (
    ContextManager,
    ExecutionContext,
    create_pipeline_context,
)
from crewai.content.agents.protocols.workflow import (
...
class TestWorkflowOrchestrator:
    ...
    def test_custom_agent_executor(self):
        """Test custom executor function."""
        agents = {"director": MockAgent("director")}

        orchestrator = WorkflowOrchestrator(agents=agents)

        # Set custom executor
        def custom_executor(agent, content, context):
            return f"custom_{content}"

        orchestrator.set_agent_executor("director", custom_executor)
        ...
        assert result.success is True
        assert result.final_output == "custom_test"

    def test_missing_agent_fails_pipeline(self):
        """Test that missing agent causes pipeline to fail."""
        agents = {"director": MockAgent("director")}

        orchestrator = WorkflowOrchestrator(agents=agents)

        result = orchestrator.execute_pipeline(
            pipeline=["director", "nonexistent"],
            initial_input="test",
        )
