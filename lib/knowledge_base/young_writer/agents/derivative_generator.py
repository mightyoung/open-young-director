...
class DerivativeContentGenerator:
    ...
    def consistency_manager(self):
        ...
        if self._consistency_manager is None:
            try:
                from young_writer.consistency import ConsistencyManager
    ...
    def _get_base_dir(self) -> Path:
        ...
        try:
            ...
            project = cfg.current_project
            if project and project.id == self.project_id:
                if cfg.generation.output_dir:
                    return Path(cfg.generation.output_dir).resolve()
                return cfg.paths.project_paths(
                    title=project.title,
        ...
        workspace_paths = WorkspacePaths.from_root(Path(__file__).resolve().parents[2])
        return workspace_paths.project_paths(
    ...
    def _get_scripts_dir(self) -> Path:
        ...
        try:
            ...
            if cfg.generation.scripts_dir:
                return Path(cfg.generation.scripts_dir).resolve()
            project = cfg.current_project
            if project and project.id == self.project_id:
                return cfg.paths.project_paths(
                    title=project.title,
