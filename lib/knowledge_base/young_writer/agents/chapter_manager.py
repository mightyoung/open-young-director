...
class ChapterManager:
    ...
    def __init__(
        ...
        longform_memory_store: Any | None = None,
    ):
        self.project_id = project_id
        self.longform_memory_store = longform_memory_store
        ...
        if override_path is not None:
            ...
            if project_dir_hint is not None and override_path.parent.name == "projects":
                runtime_dir = override_path.parent.parent
                self.root_dir = (
                    runtime_dir.parent.resolve()
        ...
        workspace_paths = WorkspacePaths.from_root(self.root_dir)
        project_slug = safe_project_slug(self.project_title, fallback=project_id)
        project_dir_basename = workspace_paths.project_basename(
            self.project_title, project_id
        )
    ...
    def _is_unified_runtime_project_dir(self, path: Path) -> bool:
        return path.parent.name == "projects"
    ...
    def _retrieve_longform_memory(self, chapter_num: int, query: str = "") -> list[dict[str, Any]]:
        """Retrieve additive long-range evidence when a memory store is injected."""
        if self.longform_memory_store is None:
            return []
        try:
            return self.longform_memory_store.retrieve(
                project_id=self.project_id,
                chapter_number=chapter_num,
                query=query,
                top_k=6,
            ...
            logger.warning("Failed to retrieve longform memory", exc_info=True)
            return []
...
def get_chapter_manager(
    ...
    return ChapterManager(
        ...
        longform_memory_store=longform_memory_store,
    )
