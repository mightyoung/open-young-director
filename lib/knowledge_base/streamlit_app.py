...
def _project_choices() -> list[str]:
    ...
    if not config_dir.exists():
        return []
    return sorted(
        file.stem.replace("project_", "") for file in config_dir.glob("project_*.json")
    )
...
def _chapter_index() -> list[dict[str, Any]]:
    ...
    for chapter_file in sorted(chapters_dir.glob("ch*.md")):
        ...
        number = (
            int(chapter_file.stem[2:5])
            if len(chapter_file.stem) >= 5 and chapter_file.stem[2:5].isdigit()
            else 0
        )
        rows.append(
            {
                ...
                "title": title,
                "word_count": int(
                    word_match.split("字数:", 1)[1].split("|", 1)[0].strip()
                )
                if "字数:" in word_match
                else len(content),
...
def _provider_status_lines() -> list[str]:
    ...
    for provider_name, payload in _provider_snapshot().items():
        marker = (
            "active"
            if provider_name == config_mgr.generation.active_provider
            else "standby"
        )
        lines.append(
...
def _chapter_detail(number: int) -> tuple[str, str, str, str]:
    ...
    report = _read_json(
        chapter_file.parent.parent
        / "consistency_reports"
        / f"ch{number:03d}_consistency.json"
    )
    plot_summary = _read_json(
        chapter_file.parent.parent / "plot_summaries" / f"ch{number:03d}_summary.json"
    )
    ...
    report_text = (
        json.dumps(report, ensure_ascii=False, indent=2) if report else "无一致性报告"
    )
    plot_text = (
...
def save_provider_settings_action(
    active_provider: str, provider_updates: dict[str, dict[str, Any]]
) -> str:
    config_mgr = get_config_manager()
...
def _launch_cli_process(run_dir: Path, cmd: list[str]) -> None:
    with (
        (run_dir / "stdout.log").open("a", encoding="utf-8") as stdout_file,
        (run_dir / "stderr.log").open("a", encoding="utf-8") as stderr_file,
    ):
...
def run_full_novel_action(
    chapters_per_volume: int, approval_mode: str, auto_approve: bool
) -> dict[str, str]:
    config_mgr = get_config_manager()
    if not config_mgr.current_project:
        return {"message": "请先创建或加载项目。"}
...
def export_text_action(start: int, end: int | None = None) -> str:
    ...
    output_dir = (
        Path(config_mgr.generation.output_dir)
        if getattr(config_mgr.generation, "output_dir", "")
        else Path(ROOT_DIR / "novels")
    )
    safe_title = config_mgr.current_project.title.replace("/", "-").strip()
    output_path = (
        output_dir / f"{safe_title}_{config_mgr.current_project.id}{EXPORT_SUFFIX}"
    )
    ...
    with st_mod.form("writing_options_form", border=True):
        ...
        option_order = [
            ...
            (
                "perspective",
                "叙事视角",
                sorted(WRITING_OPTION_GROUPS["perspective"].keys()),
            ),
            (
                "narrative_mode",
                "叙事写法",
                sorted(WRITING_OPTION_GROUPS["narrative_mode"].keys()),
            ),
            ("pace", "节奏", sorted(WRITING_OPTION_GROUPS["pace"].keys())),
            (
                "dialogue_density",
                "对白密度",
                sorted(WRITING_OPTION_GROUPS["dialogue_density"].keys()),
            ),
            (
                "prose_style",
                "行文质感",
                sorted(WRITING_OPTION_GROUPS["prose_style"].keys()),
            ),
            (
                "world_building_density",
                "设定密度",
                sorted(WRITING_OPTION_GROUPS["world_building_density"].keys()),
            ),
            (
                "emotion_intensity",
                "情绪强度",
                sorted(WRITING_OPTION_GROUPS["emotion_intensity"].keys()),
            ),
            (
                "combat_style",
                "战斗写法",
        ...
        for idx, (key, label, options) in enumerate(option_order):
            with cols[idx % 2]:
                ...
                collected[key] = st_mod.selectbox(
                    label, options=options, index=option_index, key=f"yw_{key}"
                )

        if st_mod.form_submit_button(
            "保存写作参数", use_container_width=True, type="primary"
        ):
...
def _render_provider_tab(st_mod: Any) -> None:
    ...
    active_provider = (
        config_mgr.generation.active_provider
        if config_mgr.generation.active_provider in provider_names
    ...
    with st_mod.form("provider_settings_form", border=True):
        selected_active = st_mod.selectbox(
            ...
            format_func=lambda value: (
                f"{value} · {providers[value].get('model_name', '')}"
            ),
        )
        updates: dict[str, dict[str, Any]] = {}
        for provider_name in provider_names:
            payload = providers[provider_name]
            with st_mod.expander(
                f"{provider_name} 配置", expanded=provider_name == selected_active
            ):
        ...
        if st_mod.form_submit_button(
            "保存 Provider 配置", use_container_width=True, type="primary"
        ):
            st_mod.session_state["yw_status"] = save_provider_settings_action(
                selected_active, updates
            )
            st_mod.rerun()
...
def _render_generation_tab(st_mod: Any) -> None:
    ...
    with left:
        count = st_mod.number_input(
            "生成章节数", min_value=1, max_value=20, value=1, step=1
        )
        start = st_mod.number_input(
            "起始章节",
            min_value=1,
            max_value=2000,
            value=max(config_mgr.current_project.current_chapter + 1, 1),
            step=1,
        )
    ...
    with right:
        ...
        st_mod.metric(
            "当前章节",
            f"{summary.get('current_chapter', 0)} / {summary.get('total_chapters', 0)}",
        )
        ...
        with st_mod.expander("整本长篇生成", expanded=False):
            ...
            auto_approve = st_mod.checkbox(
                "自动跳过审批", value=False, key="yw_longform_auto_approve"
            )
            if st_mod.button(
                "启动整本生成", use_container_width=True, key="yw_start_longform"
            ):
                launch = run_full_novel_action(
                    int(chapters_per_volume), approval_mode, auto_approve
                )
                st_mod.session_state["yw_generation_log"] = launch.get("message", "")
                st_mod.session_state["yw_active_run_id"] = launch.get("run_id")
                st_mod.session_state["yw_active_run_dir"] = launch.get("run_dir")
        ...
        if recent_rows:
            ...
            if preview["run_dir"]:
                ...
                if preview["latest_approval"]:
                    st_mod.caption(
                        f"最近审批: {preview['latest_approval'].splitlines()[0]}"
                    )
                if preview["approval_history"]:
                    st_mod.code(preview["approval_history"], language="text")
                st_mod.text_area(
                    "所选运行 stdout 预览", value=preview["stdout"], height=120
                )
                if preview["stderr"]:
                    st_mod.text_area(
                        "所选运行 stderr 预览", value=preview["stderr"], height=80
                    )
            if st_mod.button(
                "切换到所选运行", use_container_width=True, key="yw_switch_recent_run"
            ):
...
def _chapter_review_structured_sections(
...
def _chapter_review_evidence(review_payload: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    if goal_lock:
        ...
        goal_terms = [
            str(item).strip()
            for item in anti_drift.get("goal_terms", [])
            if str(item).strip()
        ]
        if goal_terms:
        ...
        matched_fragments = [
            str(item).strip()
            for item in anti_drift.get("matched_fragments", [])
            if str(item).strip()
        ]
        ...
        unaligned_fragments = [
            str(item).strip()
            for item in anti_drift.get("unaligned_fragments", [])
            if str(item).strip()
        ]
    ...
    if bridge_results:
        ...
        unbridged_fragments = [
            str(item).strip()
            for item in anti_drift.get("unbridged_fragments", [])
            if str(item).strip()
        ]
    ...
    if chapter_intent_contract:
        lines = []
        planned_action = str(
            chapter_intent_contract.get("planned_action", "") or ""
        ).strip()
        if planned_action:
            lines.append(f"- 本章计划动作: {planned_action}")
        contract_goal_lock = str(
            chapter_intent_contract.get("goal_lock", "") or ""
        ).strip()
        if contract_goal_lock:
            lines.append(f"- 合同目标锁: {contract_goal_lock}")
        success_checks = [
            str(item).strip()
            for item in chapter_intent_contract.get("success_checks", [])
            if str(item).strip()
        ]
        if success_checks:
            lines.append("- 执行合同:")
    ...
    if rewrite_plan:
        ...
        fixes = [
            str(item).strip()
            for item in rewrite_plan.get("fixes", [])
            if str(item).strip()
        ]
        success_criteria = [
            str(item).strip()
        ...
        operations = [
            item
            for item in rewrite_plan.get("operations", [])
...
def _chapter_review_resume_payload(
    review_payload: dict[str, Any], extra_notes: str
) -> dict[str, Any]:
...
def _volume_review_summary(payload: dict[str, Any]) -> str:
    ...
    registry_summary = str(
        review_payload.get("cross_volume_registry_summary", "") or ""
    ).strip()
    if registry_summary:
    ...
    excerpts = review_payload.get("chapter_evidence_excerpts", []) or []
    if excerpts:
        lines.extend(["", "正文证据摘录:"])
        for item in excerpts[:3]:
            lines.append(
                f"- 第 {item.get('chapter_number', 0)} 章《{item.get('title', '')}》开篇: "
                f"{item.get('opening_excerpt', '')}"
            )
            closing = str(item.get("closing_excerpt", "") or "").strip()
            if (
                closing
                and closing != str(item.get("opening_excerpt", "") or "").strip()
            ):
...
def _volume_registry_defaults(review_payload: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        "unresolved_goals": "\n".join(
            str(item).strip()
            for item in registry.get("unresolved_goals", [])
            if str(item).strip()
        ),
        "open_promises": "\n".join(
            str(item).strip()
            for item in registry.get("open_promises", [])
            if str(item).strip()
        ),
        "dangling_settings": "\n".join(
            str(item).strip()
            for item in registry.get("dangling_settings", [])
            if str(item).strip()
        ),
    }
...
def _volume_guidance_draft(review_payload: dict[str, Any]) -> dict[str, str]:
    ...
    for item in highlights:
        ...
        if key_events and not relationship_focus:
            relationship_focus = (
                "围绕本卷后段关键事件继续施压相关人物关系：" + "；".join(key_events[:2])
            )
        if summary and not tone_target:
            if any(
                marker in summary
                for marker in ("反转", "危机", "死守", "追击", "冲突", "压迫")
            ):
...
def _longform_control_panel_sections(status: dict[str, Any]) -> list[tuple[str, str]]:
    ...
    for label, key in (
        ...
        values = [
            str(item).strip() for item in registry.get(key, []) if str(item).strip()
        ]
        if values:
    ...
    if pending_state_path:
        ...
        review_payload = (
            pending_payload.get("review_payload", {})
...
def _render_run_status_summary(st_mod: Any, payload: dict[str, Any]) -> None:
    ...
    if status.get("failed_stage") or status.get("error_message"):
        ...
        pause_reason = _pause_reason_label(
            status.get("pause_reason") or ""
...
def _render_run_monitor(st_mod: Any) -> None:
    ...
    if st_mod.button(
        "刷新运行状态", use_container_width=True, key="yw_refresh_run_state"
    ):
        st_mod.rerun()
...
def _render_pending_review(st_mod: Any) -> None:
    ...
    if checkpoint_type == "outline_review":
        with st_mod.form("yw_outline_review_form", border=True):
            outline = st_mod.text_area(
                "大纲", value=review_payload.get("outline", ""), height=140
            )
            world_setting = st_mod.text_area(
                "世界观", value=review_payload.get("world_setting", ""), height=120
            )
            character_intro = st_mod.text_area(
                "人物设定", value=review_payload.get("character_intro", ""), height=120
            )
            approve = st_mod.form_submit_button(
                "批准并继续", use_container_width=True, type="primary"
            )
            revise = st_mod.form_submit_button(
                "保存修改后继续", use_container_width=True
            )
    ...
    if checkpoint_type == "volume_review":
        volume_index = review_payload.get(
            "volume_index", payload.get("current_volume", 0)
        )
        ...
        tone_target = st_mod.text_input(
            "目标基调", value=guidance_draft["tone_target"], key="yw_volume_tone_target"
        )
        ...
        guidance_payload = {
            ...
            "extra_notes": extra_notes,
            "unresolved_goals": [
                line.strip() for line in unresolved_goals.splitlines() if line.strip()
            ],
            "open_promises": [
                line.strip() for line in open_promises.splitlines() if line.strip()
            ],
            "dangling_settings": [
                line.strip() for line in dangling_settings.splitlines() if line.strip()
            ],
        }
        preview_lines = [
        ...
        if any(
            ...
            for value in guidance_payload.values()
        ):
        ...
        if cols[0].button(
            ...
            st_mod.rerun()
        if cols[1].button(
            "带备注继续", use_container_width=True, key="yw_volume_revise"
        ):
            result = resume_longform_action(
                run_dir,
                payload["pending_state_path"],
    ...
    if checkpoint_type == "risk_review":
        ...
        notes = st_mod.text_area(
            "风险复核备注", value="", height=120, key="yw_risk_review_notes"
        )
        cols = st_mod.columns(3)
        if cols[0].button(
            "确认继续", use_container_width=True, type="primary", key="yw_risk_approve"
        ):
            result = resume_longform_action(
                run_dir,
                payload["pending_state_path"],
            ...
            st_mod.rerun()
        if cols[1].button(
            "记录备注后继续", use_container_width=True, key="yw_risk_revise"
        ):
            result = resume_longform_action(
                run_dir,
                payload["pending_state_path"],
    ...
    if checkpoint_type == "chapter_review":
        ...
        for label, body in _chapter_review_evidence(review_payload):
            with st_mod.expander(
                label, expanded=label in {"结构化重写方案", "目标锁证据"}
            ):
                st_mod.code(body, language="text")
        ...
        if cols[0].button(
            ...
            st_mod.rerun()
        if cols[1].button(
            "带备注重试", use_container_width=True, key="yw_chapter_revise"
        ):
            result = resume_longform_action(
                run_dir,
                payload["pending_state_path"],
            ...
            st_mod.rerun()
        if cols[2].button(
            "保持暂停", use_container_width=True, key="yw_chapter_reject"
        ):
            result = resume_longform_action(
                run_dir,
                payload["pending_state_path"],
...
def _render_chapters_tab(st_mod: Any) -> None:
    ...
    current_label = next(
        (label for label, number in choices if int(number) == current_number),
        chapter_labels[0],
    )
    selected_label = st_mod.selectbox(
        "选择章节",
        options=chapter_labels,
        index=chapter_labels.index(current_label),
        key="yw_chapter_selector",
    )
...
def _render_export_tab(st_mod: Any) -> None:
    ...
    with cols[1]:
        end = st_mod.number_input(
            "导出结束章",
            min_value=1,
            max_value=2000,
            value=max(config_mgr.current_project.current_chapter, 1),
            step=1,
            key="yw_export_end",
        )
    with cols[2]:
        if st_mod.button("导出文本", use_container_width=True):
            st_mod.session_state["yw_export_message"] = export_text_action(
                int(start), int(end)
            )
            st_mod.rerun()
...
def build_app() -> None:
    ...
    st_mod.markdown(
        '<div class="yw-title">小说生成工作台</div>', unsafe_allow_html=True
    )
    st_mod.markdown(
