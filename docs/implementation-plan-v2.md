...
class NovelCrew(BaseContentCrew):
    ...
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(content_type="novel", config=config, **kwargs)
        ...
        self.dianting_checker = None  # 延迟初始化
    ...
    def kickoff(self) -> NovelOutput:
        """执行小说生成"""
        # 1. World Building
        world_output = self._run_world_crew()

        # 2. Outline Planning
        outline = self._run_outline_crew(world_output)

        # 3. Per-chapter writing loop
        chapters = []
        ...
        return NovelOutput(
            title=outline.title,
            chapters=chapters,
            world_settings=world_output,
            outline=outline,
            dianting_report=self.dianting_checker.generate_report() if self.dianting_checker else None
        )
